...
def find_reasonable_e(U_func, log_q, thetas, M_inv, y):
    ...
    p = momentum.sample()
    log_q = log_q.clone()
    log_q_prime, p_prime, thetas_prime = leapfrog(U_func, log_q, p, thetas, epsilon, M_inv, y, init=True)
    log_q_prime = log_q_prime.clone().detach().requires_grad_(True)
    while torch.isinf(U_func(log_q_prime, thetas_prime, y)) or torch.isnan(U_func(log_q_prime, thetas_prime, y)):
        k *= 0.5
        log_q_prime, p_prime, thetas_prime = leapfrog(U_func, log_q, p, thetas, epsilon * k, M_inv, y, init=True)
    epsilon *= 0.5 * k
...
def build_tree(U_func, log_q, p, thetas, direction, depth, epsilon, log_q_0, p_0, M_inv, y, delta_max=1000):
    if depth == 0:
        log_q_prime, p_prime = leapfrog(U_func, log_q, p, thetas, direction * epsilon, M_inv, y)
        ...
        return log_q_prime, p_prime, log_q_prime, p_prime, log_q_prime, s_prime, alpha_prime, n_alpha_prime, log_sum_tree
    else:
        log_q_minus, p_minus, log_q_plus, p_plus, log_q_prime, s_prime, alpha_prime, n_alpha_prime, log_sum_tree = build_tree(U_func, log_q, p, thetas, direction, depth - 1, epsilon, log_q_0, p_0, M_inv, y)
        if s_prime:
            if direction == -1:
                log_q_minus, p_minus, _, _, log_q_double_prime, s_double_prime, alpha_double_prime, n_alpha_double_prime, log_sum_tree_prime = build_tree(U_func, log_q_minus, p_minus, thetas, direction, depth - 1, epsilon, log_q_0, p_0, M_inv, y)
            else:
                _, _, log_q_plus, p_plus, log_q_double_prime, s_double_prime, alpha_double_prime, n_alpha_double_prime, log_sum_tree_prime = build_tree(U_func, log_q_plus, p_plus, thetas, direction, depth - 1, epsilon, log_q_0, p_0, M_inv, y)
            ...
            s_prime = bool(s_double_prime and ((log_q_plus - log_q_minus) @ p_minus).item() >= 0 and (
                    (log_q_plus - log_q_minus) @ p_plus).item() >= 0)
        return log_q_minus, p_minus, log_q_plus, p_plus, log_q_prime, s_prime, alpha_prime, n_alpha_prime, log_sum_tree
...
def NUTS(U_func, log_alpha, log_beta, num_samples, m_adapt=1000):
    ...
    initial_thetas[:509] = conditional_posterior_theta_uncensored(q[0].clone(), q[1].clone(),
                                                                    y_uncensored).squeeze()
    initial_thetas[509:] = conditional_posterior_theta_censored(q[0].clone(), q[1].clone(),
    ...
    for i in range(num_samples):
        ...
        if i % 100 == 0:
            ...
            print("alpha: " + str(torch.exp(log_q[0]).item()) + " beta: " + str(torch.exp(log_q.clone()[1]).item()))
            print(epsilon)

        theta_samples[:509, i] = conditional_posterior_theta_uncensored(q[0].clone(), q[1].clone(),
                                                                        y_uncensored).squeeze()
        theta_samples[509:, i] = conditional_posterior_theta_censored(q[0].clone(), q[1].clone(),
                                                                      y_censored_size).squeeze()

        momentum = MultivariateNormal(torch.zeros(2), covariance_matrix=M)
        thetas = torch.tensor(theta_samples[:, i], requires_grad=False, dtype=torch.float32)
        ...
        while s and depth < 10:
            ...
            if v == -1:
                log_q_minus, p_minus, _, _, log_q_prime, s_prime, alpha, n_alpha, log_sum_tree_prime = build_tree(U_func, log_q_minus, p_minus, thetas, v, depth, epsilon, log_q_0, p_0, M_inv, y)
            else:
                _, _, log_q_plus, p_plus, log_q_prime, s_prime, alpha, n_alpha, log_sum_tree_prime = build_tree(U_func, log_q_plus, p_plus, thetas, v, depth, epsilon, log_q_0, p_0, M_inv, y)
            ...
            if s_prime:
                if log_sum_tree_prime > log_sum_total:
                    ...
                    if np.random.uniform() < acceptance:
                        log_q = log_q_prime
            log_sum_tree = log_sum_total
            s = bool(s_prime and ((log_q_plus - log_q_minus) @ p_minus).item() >= 0 and ((log_q_plus - log_q_minus) @ p_plus).item() >= 0)
            depth = depth + 1
        if i <= m_adapt:
